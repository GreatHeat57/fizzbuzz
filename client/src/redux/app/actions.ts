...
export const setFizzBuzz = (fizzbuzz: string) => ({
  type: Action.SET_FIZZBUZZ,
  payload: fizzbuzz,
});
...
export const getFizzBuzz = (count: number) => {
  return async (dispatch: (arg0: any) => void) => {
    try {
      ...
      if (res.data.status === 200) dispatch(setFizzBuzz(res.data.result));
      else dispatch(setFizzBuzz('error'));
    } catch (e) {
      console.log(e, 'error');
    }
  };  
